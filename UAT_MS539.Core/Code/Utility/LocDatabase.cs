...
namespace UAT_MS539.Core.Code.Utility
{
    ...
    public class LocDatabase
    {
        ...
        private string _currentLangId = System.Globalization.CultureInfo.CurrentCulture.TwoLetterISOLanguageName;
        ...
        {
            HashSet<string> langHash = new HashSet<string>(10);
            Dictionary<string, LocEntry> entryDict = new Dictionary<string, LocEntry>(locEntries.Length);
            foreach (LocEntry locEntry in locEntries)
            {
        ...
        public LocDatabase(string jsonDataPath)
        {
            ...
            HashSet<string> langHash = new HashSet<string>(10);
            foreach (LocEntry locEntry in LocEntryById.Values)
            {
                langHash.UnionWith(locEntry.TextByLang.Keys);
            }
        ...
        {
            return TryLocalize(locId, out string localizedText, locParams) ? localizedText : $"MISSING LOC: {locId}";
        }
        ...
        public bool TryLocalize(string locId, out string localizedText, IReadOnlyDictionary<string, string> locParams = null)
        {
            ...
            if (string.IsNullOrWhiteSpace(CurrentLangId))
                return false;
            if (!LocEntryById.TryGetValue(locId, out LocEntry locEntry))
                return false;
